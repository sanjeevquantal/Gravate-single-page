...
from schemas.match import MatchCreate, ScoreUpdate
...
class MatchService:
    ...
    @staticmethod
    def update_match_score(db: Session, match_id: int, score_update: ScoreUpdate) -> Optional[Match]:
    ...
    @staticmethod
    def complete_match(db: Session, match_id: int) -> Optional[Match]:
        db_match = db.query(Match).filter(Match.id == match_id).first()
        if not db_match:
            return None
